...
public class MarkdownHeader: MarkdownLevelElement {
  ...
  public init(maxLevel: Int = 0, font: UIFont? = UIFont.boldSystemFontOfSize(UIFont.smallSystemFontSize()),
       color: UIColor? = nil) {
  ...
  public func attributesForLevel(level: Int) -> [String: AnyObject] {
    ...
    if let font = font {
      let headerFontSize: CGFloat = font.pointSize + (CGFloat(level) * 2)
